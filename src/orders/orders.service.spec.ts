...
describe('OrdersService', () => {
  ...
  it('any role can addItemToOrder', async () => {
    ...
    jest.spyOn(prisma.menuItem, 'findUnique').mockResolvedValue(mockMenuItem);
    jest.spyOn(prisma.orderItem, 'create').mockResolvedValue({} as any);
    jest
      .spyOn(prisma.order, 'update')
      .mockResolvedValue({ ...mockOrder, totalAmount: 300.0 } as any);
  ...
  it('MEMBER cannot cancelOrder → throws ForbiddenException', async () => {
    await expect(service.cancel('order-id', mockUserMember)).rejects.toThrow(
      ForbiddenException,
    );
  });
  ...
  it('MANAGER can placeOrder', async () => {
    ...
    jest
      ...
      .mockResolvedValue(managerOrder as any);
    jest
      .spyOn(prisma.paymentMethod, 'findUnique')
  ...
  it("user cannot access another user's order → throws ForbiddenException", async () => {
    ...
    await expect(service.findOne('order-id', mockUserOther)).rejects.toThrow(
      ForbiddenException,
    );
  });
});
